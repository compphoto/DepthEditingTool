...
export const runSaveDepthOperations = image => {
  const canvas = document.createElement("canvas");
  canvas.width = image.width;
  canvas.height = image.height;
  const context = canvas.getContext("2d");
  let stack = store.getState().image.operationStack["saveStack"];
  stack.forEach(element => {
    element.params ? element.func(image, context, ...element.params) : element.func(image, context);
  });
  const storeAction = require("store/store");
  let saveBitmapCanvas = store.getState().image.saveBitmapCanvas;
  let data = {
    savedDepthCanvas: cloneCanvas(canvas),
    saveBitmapCanvas: saveBitmapCanvas === null ? cloneCanvas(canvas) : saveBitmapCanvas
  };
  storeAction.default.dispatch(imageActions.initImage(data));
};
